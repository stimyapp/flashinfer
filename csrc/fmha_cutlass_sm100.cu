...
// FP4KV dispatch: Q is always FP8 E4M3, output is always BF16
...
void FMHACutlassSM100RunFP4KV(
  ...
  int num_kv_heads = k.size(1);
  int head_dim_qk = q.size(2);
  // k.size(2) is head_dim/2 (packed E2M1), so head_dim_vo = head_dim_qk for now
  // v_sf.size(2) is head_dim/16
  int head_dim_vo = head_dim_qk;  // For E2M1 KV, head_dim_qk == head_dim_vo
  int batch_size = qo_segment_offsets.size(0) - 1;
  int q_stride_n = q.stride(0);
  int q_stride_h = q.stride(1);

  // E2M1 packed strides: [total_kv, num_kv_heads, head_dim/2]
  int k_e2m1_stride_n = k.stride(0);
  int k_e2m1_stride_h = k.stride(1);
  int v_e2m1_stride_n = v.stride(0);
  int v_e2m1_stride_h = v.stride(1);

  // Scale factor strides: [total_kv, num_kv_heads, head_dim/16]
  ...
  DISPATCH_mask_mode(mask_mode, MASK_MODE, [&] {
    return DISPATCH_head_dim_fp4kv(head_dim_qk, head_dim_vo, HEAD_DIM_QK, HEAD_DIM_VO, [&] {
      ...
      auto status = run_fmha_fwd_fp4kv<cutlass_type_q, cutlass_type_out, int32_t,
      ...
      TVM_FFI_ICHECK_EQ(status, cudaSuccess)
          << "Cutlass FP4KV FMHA forward pass failed: " << cudaGetErrorString(status);
